...
void PlayerMovementController::lookAround(const InputManager& input)
{
	...
	if (input.GetThumbRPosition().y > 0.3f)
	{
		...
		m_rigidBody->setRotation(m_rigidBody->getRotation() *
			glm::quat(glm::cos(0.5f * 1.0f * 3.14159625f / 180.0f),
			glm::sin(0.5f * 1.0f * 3.14159265f / 180.0f), 0, 0));
...
void PlayerMovementController::returnToActualRotation()
{
	if (glm::abs(m_shipsVisualRotation.x) != 0 || glm::abs(m_shipsVisualRotation.y) != 0)
	{
		m_rigidBody->setRotation(m_rigidBody->getRotation() *
			glm::quat(
			glm::cos(0.5f * -(m_shipsVisualRotation.x + m_shipsVisualRotation.y) * 3.14159625f / 180.0f),
			glm::sin(0.5f * -m_shipsVisualRotation.x * 3.14159265f / 180.0f),
			glm::sin(0.5f * -m_shipsVisualRotation.y * 3.14159265f / 180.0f),
			0
			));
	}
}
