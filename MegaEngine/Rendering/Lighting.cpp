...
ShadowCameraTransform BaseLight::calcShadowCameraTransform(
	const glm::vec3 & mainCameraPos, 
	const glm::quat & mainCameraRot) const
{
...
ShadowCameraTransform DirectionalLight::calcShadowCameraTransform(
	const glm::vec3 & mainCameraPos, 
	const glm::quat & mainCameraRot) const
{
	glm::vec3 resultPos = 
		mainCameraPos + Utility::getForward(mainCameraRot) * getHalfShadowArea();
	
	glm::quat resultRot = getTransform().getTransformedRot();

	float worldTexelSize = 
		(getHalfShadowArea() * 2) / ((float)(1 << getShadowInfo().getShadowMapSizeAsPowerOf2()));

	glm::vec3 lightSpaceCameraPos = 
		Utility::rotateQuatByVec(glm::conjugate(resultRot), resultPos);
	
	lightSpaceCameraPos.x = 
		(worldTexelSize * glm::floor(lightSpaceCameraPos.x / worldTexelSize));
	
	lightSpaceCameraPos.y = 
		(worldTexelSize * glm::floor(lightSpaceCameraPos.y / worldTexelSize));
